...
const { decodeAirQualityRows, airQualityOverview, estimateNeighborhoods, queryAirQualityRows } = require('./air-quality-query');
...
const FEEDS = {
  ...
  air_quality: { route: '/api/air-quality',
    rows: d => decodeAirQualityRows(d.rows, { nowMs: Date.now() }).map(r => ({ ...r, label: r.name + ' · ' + r.displayPm25 + ' µg/m³' + (r.nowcastAqi == null ? '' : ' · AQI ' + r.nowcastAqi) })),
    search: r => r.name + ' ' + r.siteId + ' NYCCAS',
    summary: d => { const rows = decodeAirQualityRows(d.rows, { nowMs: Date.now() }); return { monitorCount: rows.length, currentMonitorCount: rows.filter(r => r.freshness === 'current').length, officialAirNowAvailable: !!(d.airNow && d.airNow.current), citation: d.citation }; } },
...
async function airQualityQuery(input = {}) {
  const raw = await readFeed('/api/air-quality'), route = routes.get('/api/air-quality'), nowMs = Date.now();
  const data = { ...raw, fetchedAt: route && route.fetchedAt, stale: !route || route.data === null || nowMs - route.fetchedAt >= route.ttl + 5000 };
  const sensors = decodeAirQualityRows(data.rows, { nowMs });
  const view = ['overview', 'sensors', 'neighborhoods'].includes(input.view) ? input.view : 'overview';
  if (view === 'overview') return airQualityOverview(data, sensors, nowMs);
  let rows;
  if (view === 'neighborhoods') {
    const B = await boundaries();
    rows = estimateNeighborhoods(B.neighborhoods, sensors, distM);
  } else rows = sensors;
  let area = null;
  if (input.area) {
    if (view === 'neighborhoods') {
      const B = await boundaries(), borough = B.boroughs.find(item => bnorm(item.name) === bnorm(input.area)) || null;
      const region = borough ? null : await findRegion('neighborhood', input.area);
      if (!region && !borough) return { error: 'unknown area "' + input.area + '" — use a 2020-NTA neighborhood or borough name' };
      area = borough ? borough.name : region.name;
      rows = borough ? rows.filter(row => bnorm(row.borough) === bnorm(borough.name)) : rows.filter(row => bnorm(row.name) === bnorm(region.name));
    } else {
      const scoped = await filterByArea(rows, input.area);
      if (scoped.error) return scoped;
      rows = scoped.rows; area = scoped.area;
    }
  }
  if (input.near && isFinite(input.near.lat) && isFinite(input.near.lon)) {
    const radius = Math.min(Math.max(Number(input.near.radius_m) || 5000, 50), 25000);
    rows = rows.map(row => ({ ...row, dist_m: Math.round(distM(input.near.lat, input.near.lon, row.lat, row.lon)) })).filter(row => row.dist_m <= radius);
    if (!input.sort_by) input = { ...input, sort_by: 'distance', descending: false };
  }
  if ((input.sort_by === 'distance' || input.sort_by === 'name') && input.descending == null) input = { ...input, descending: false };
  const queried = queryAirQualityRows(rows, { top: 12, ...input, view }, row => row.name + ' ' + (row.siteId || '') + ' ' + (row.borough || ''));
  return {
    view, ...(area ? { area } : {}), matched: queried.matched, rows: queried.rows,
    measuredAt: data.observedAt ? new Date(data.observedAt).toISOString() : null,
    note: view === 'neighborhoods' ? 'NYC SIM live estimate from up to four current NYCCAS monitors using inverse-distance-squared weighting; not official AirNow neighborhood data' : 'Measured NYCCAS PM2.5; station NowCast AQI is derived by NYC SIM using the EPA method',
    citation: data.citation || 'NYC Health + Queens College — NYCCAS'
  };
}
...
async function buildLayer(input) {
  ...
  } else if (src.air_quality) {
    const query = { view: 'sensors', ...src.air_quality, top: Math.min(Number(src.air_quality.top) || 120, 200) };
    const res = await airQualityQuery(query);
    if (res.error) return res;
    for (const r of res.rows || []) if (typeof r.lat === 'number') {
      const value = query.view === 'neighborhoods' ? r.estimatedAqi : r.nowcastAqi;
      layer.points.push({ lat: r.lat, lon: r.lon, label: String(r.name + (value == null ? '' : ' · AQI ' + value)).slice(0, 48) });
    }
  ...
  } else return { error: 'source needs feed | air_quality | buildings | street | boundary | choropleth | points' };
...
const TOOLS = [
  ...
  { name: 'get_air_quality',
    description: 'Current air quality. view=overview returns official AirNow NYC-region current AQI, forecast, Action Day, NYCCAS coverage, and source freshness. view=sensors returns measured NYCCAS monitors. view=neighborhoods returns live NYC SIM estimates interpolated from up to four current monitors; these are NOT official neighborhood AQI. Supports name filter, real borough/NTA area, proximity, ranking, and top.',
    input_schema: { type: 'object', properties: {
      view: { type: 'string', enum: ['overview', 'sensors', 'neighborhoods'] }, filter: { type: 'string' }, area: { type: 'string' },
      near: { type: 'object', properties: { lat: { type: 'number' }, lon: { type: 'number' }, radius_m: { type: 'number' } } },
      sort_by: { type: 'string', enum: ['aqi', 'pm25', 'latest', 'change', 'peak', 'age', 'distance', 'name'] },
      descending: { type: 'boolean' }, top: { type: 'integer' } } } },
  { name: 'get_history',
    description: 'Recorded history. No args lists available city days and 30-day Air Quality days. With day (YYYY-MM-DD), returns the daily city snapshot; feed=airQuality returns exact packed NYCCAS hourly history plus official daily AirNow regional AQI. Optional hour is New York local hour 0-23; without it the day-ending AQ frame is used.',
    input_schema: { type: 'object', properties: { day: { type: 'string' }, feed: { type: 'string' }, hour: { type: 'integer', minimum: 0, maximum: 23 } } } },
  ...
  { name: 'show_layer',
    ...
    input_schema: { type: 'object', properties: {
      ...
      source: { type: 'object', properties: {
        ...
        air_quality: { type: 'object', description: 'map air-quality monitor or estimated-neighborhood results', properties: {
          view: { type: 'string', enum: ['sensors', 'neighborhoods'] }, filter: { type: 'string' }, area: { type: 'string' },
          near: { type: 'object' }, sort_by: { type: 'string' }, descending: { type: 'boolean' }, top: { type: 'integer' } } },
...
NYC Ferry, MTA buses, Citi Bike, DOT traffic speeds/cameras/incidents, weather, AirNow regional air \
quality, measured NYCCAS PM2.5 monitors, estimated neighborhood air quality, BirdCast bird migration, \
and LinkNYC Wi-Fi kiosks (feed=linknyc, static locations) — plus the real street graph \
...
- For air quality, use get_air_quality. AirNow is the official NYC-region current/forecast source; NYCCAS \
rows are measured PM2.5; station NowCast is NYC SIM-derived with the EPA method; neighborhood AQI is \
a live NYC SIM interpolation, never official AirNow neighborhood data. State neighborhood confidence, \
use µg/m³ for PM2.5, and describe delayed/overdue as data age—not monitor connectivity. For health \
questions report the EPA category and point to official guidance, not individualized medical advice.
...
async function historyTool(input) {
  if (!input || !input.day) {
    ...
    return { retentionDays: man.retentionDays, days: (man.daily || []).map(d => d.replace(/^daily\//, '').replace(/\.json$/, '')),
      airQualityRetentionDays: man.airQualityRetentionDays, airQualityDays: (man.airQuality || []).map(d => d.replace(/^air-quality\//, '').replace(/\.json$/, '')) };
  }
  if (!/^\d{4}-\d{2}-\d{2}$/.test(input.day)) return { error: 'day must be YYYY-MM-DD' };
  if (input.feed === 'airQuality') {
    const pack = await viaHandleApi('/api/history/air-quality/' + input.day);
    if (pack.error) return pack;
    const localHour = Number.isInteger(input.hour) ? input.hour : null;
    const hourFmt = new Intl.DateTimeFormat('en-US', { timeZone: 'America/New_York', hourCycle: 'h23', hour: 'numeric' });
    let frame = null;
    if (localHour == null && Number.isFinite(+pack.daily)) frame = (pack.frames || []).find(item => +item[0] === +pack.daily) || null;
    else frame = (pack.frames || []).find(item => +hourFmt.format(new Date(+item[0] * 60000 - 1)) === localHour) || null;
    return { day: input.day, intervalEndedAt: frame ? new Date(+frame[0] * 60000).toISOString() : null,
      rows: frame ? decodeAirQualityRows(frame[1], { recorded: true, nowMs: +frame[0] * 60000 }) : [], officialAirNow: pack.airNow || null,
      note: frame ? 'Exact recorded NYCCAS hourly interval; neighborhood estimates can be reconstructed but remain nonofficial. AirNow is the official daily NYC reporting-area observation.' : 'No recorded NYCCAS reading for that interval; values were not carried forward.' };
  }
  ...
  const out = { day: input.day, recordedAt: fr.t, weather: fr.weather, counts: {
    ...
    trafficLinks: (fr.traffic || []).length, trafficEvents: (fr.trafficEvents || []).length,
    airQualityMonitors: (fr.airQuality || []).length }, schema: fr.schema };
  const feed = input.feed;
  if (feed && fr[feed]) out.rows = fr[feed].slice(0, 30);
  return out;
}
...
async function runTool(name, input) {
  try {
    ...
    if (name === 'get_air_quality') return await airQualityQuery(input || {});
...
module.exports = { handleAgent, feedQuery, airQualityQuery, buildLayer, historyTool, findPlace, whereIs, placesKey }; // feedQuery for tests; findPlace/whereIs/placesKey shared with lib/home-core.js (same key/budget/cache rails)
