...
const TOOLS = [
  ...
  { name: 'get_history',
    description: 'Recorded daily snapshots (last ~7 days, one frame per day ~05:05 UTC). No args → list of days. With day (YYYY-MM-DD) → weather + counts; add feed (buses|bikes|ferries|flights|traffic|trafficEvents|airQuality) for sample rows. airQuality history contains recorded NYCCAS monitors only, not historical AirNow forecasts or neighborhood estimates.',
...
async function historyTool(input) {
  ...
  if (feed === 'airQuality' && fr.airQuality) {
    out.rows = decodeAirQualityRows(fr.airQuality, { recorded: true, nowMs: Number(fr.t) || Date.parse(input.day) });
    out.note = 'Recorded NYCCAS monitor snapshot; historical AirNow forecasts and neighborhood estimates were not recorded.';
  } else if (feed && fr[feed]) out.rows = fr[feed].slice(0, 30);
  return out;
}
