...
async function handleAgent(req, res) {
  ...
  try {
    ...
    if (/abort|timeout/i.test(String(e.message || e)) || e.name === 'TimeoutError') {
      ...
      return sendJSON(res, { reply: reply || 'That one took too long on the model side — please ask again (it usually goes through on the second try).',
        camera, layers: layers.length ? layers : null, timeline });
    }
