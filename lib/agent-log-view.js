...
/* wrong-key throttle: the viewer key is a human password, so make online guessing
   impractical — 10 misses per IP per hour, then 429. Only FAILED attempts count
   (the live viewer polls with the good key every 30 s and must never trip this).
   Warm-instance memory, same tradeoff as the concierge limiter in agent-core. */
const BAD = new Map(); // ip → [ms of failed attempts inside the window]
const BAD_MAX = 10, BAD_WINDOW_MS = 3600_000;
function authThrottled(ip) {
  const now = Date.now();
  const arr = (BAD.get(ip) || []).filter(t => now - t < BAD_WINDOW_MS);
  BAD.set(ip, arr);
  if (BAD.size > 5000) BAD.clear();
  return arr.length >= BAD_MAX;
}
function noteBadKey(ip) { BAD.get(ip).push(Date.now()); } // authThrottled always ran first → entry exists
...
async function handleAgentLog(req, res) {
  ...
  if (url.searchParams.get('key') !== key) { noteBadKey(ip); return sendJSON(res, { error: 'unauthorized' }, 401); }
  ...
  try {
    if (days) {
      ...
      const found = new Set();
      do {
        const page = await listLogs('agent-logs/', 1000, cursor);
        for (const blob of page.blobs || []) {
          const match = String(blob.pathname || '').match(/^agent-logs\/(\d{4}-\d{2}-\d{2})\//);
          if (match) found.add(match[1]);
        }
        cursor = page.hasMore ? page.cursor : null;
        pages++;
      } while (cursor && pages < 1000);
      return sendJSON(res, { days: [...found].sort().reverse() });
    }
