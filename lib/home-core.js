...
const { findPlace, whereIs } = require('./agent-core');
...
function setCookie(req, res, value, maxAgeS) {
  ...
  res.setHeader('Set-Cookie',
    `${COOKIE}=${value}; Max-Age=${maxAgeS}; Path=/api/home; HttpOnly; SameSite=Lax${secure}`);
}
...
async function readProfile(subH) {
  const hit = profCache.get(subH);
  if (hit && Date.now() - hit.t < PROF_TTL) return hit.profile;
...
async function handleHome(req, res) {
  ...
  if (req.method !== 'POST') {
    ...
    let profile = null;
    try { profile = await readProfile(subH); } catch { /* transient blob issue → boot signed-in, home unknown */ }
